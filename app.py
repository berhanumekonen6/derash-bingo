...
st.markdown(f"""
<div style="background:linear-gradient(135deg,rgba(255,215,0,0.08),rgba(255,215,0,0.03));border-left:4px solid #FFD700;padding:12px 18px;border-radius:10px;margin:10px 0;backdrop-filter:blur(10px);">
    <div style="font-size:1rem;color:#FFD700;font-style:italic;font-family:'Noto Sans Ethiopic',Arial,sans-serif;">"{quote['am']}"</div>
    <div style="color:rgba(255,255,255,0.4);font-size:0.8rem;margin-top:3px;">{quote['en']} — {quote['author']}</div>
</div>
""", unsafe_allow_html=True)
...
if not st.session_state.selected_card:
    ...
    # Numbers grid - 6 columns with circle buttons
    cols = st.columns(6)
    ...
    if len(st.session_state.clicked_numbers) >= 2:
        ...
        st.info("👆 Click a circle number to select it (max 2 cards)")
