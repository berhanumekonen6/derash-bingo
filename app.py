...
def sync_global_winners():
    ...
    if winner_declared:
        ...
        # ✅ Do NOT pull the shared timestamp — each player starts their own
        # celebration window locally.
        return True
    return False
...
CARD_PRICE = 10
PRIZE_PER_CARD = 8
CELEBRATION_DURATION = 3  # Kept for reference — Resume button controls reset now
...
def try_global_call():
    """✅ Immediately returns None if a winner already exists — 
       so NO new numbers are called once a winner exists."""
...
def distribute_prizes(winners):
    """✅ Splits the pot EQUALLY among all winners and updates each balance."""
...
def render_card_selection():
    ...
    progress = 1 - (remaining / 60) if remaining > 0 else 0
...
# ===================================================================
# ✅ WINNER CELEBRATION — WAITS FOR RESUME BUTTON (NO AUTO-RESET)
# ===================================================================
if st.session_state.winner_declared:
    if st.session_state.celebration_start_time is None:
        st.session_state.celebration_start_time = time.time()
    # ✅ No auto-reset here — the Resume button on the winner screen handles it.
    ...
    if st.session_state.game_started:
        if st.session_state.winner_declared:
            ...
            <div style="background:rgba(255,215,0,0.1);border:2px solid #FFD700;border-radius:15px;padding:20px;text-align:center;margin:20px 0;">
                ...
                <p style="color:rgba(255,255,255,0.8);">Winners are being displayed on player pages...</p>
            </div>
            """, unsafe_allow_html=True)
...
if st.session_state.game_started:
    ...
    # ✅ FALLBACK: Rebuild this user's own cards from the shared file
    if not all_player_cards and st.session_state.current_user:
        _gt, _go, _, _ = load_global_cards()
        _recovered = set()
        for _cid_str, _owner in _go.items():
            if _owner == st.session_state.current_user:
                try:
                    _recovered.add(int(_cid_str))
                except (ValueError, TypeError):
                    pass
        if _recovered:
            st.session_state.clicked_numbers = _recovered
            all_player_cards = list(_recovered)
    
    # ==============================================================
    # ✅ WINNER DECLARED — CELEBRATION ON EVERY PLAYER'S PAGE
    # ==============================================================
    if st.session_state.winner_declared:
        sync_global_winners()
        ...
        # ✅ RESUME BUTTON — player clicks to return to card selection
        st.markdown("""
    ...
    # ==============================================================
        ...
        if st.session_state.current_user:
            ...
            for _cid_str, _owner in _go.items():
                if _owner == st.session_state.current_user:
                    try:
                        _my_cards.append(int(_cid_str))
                    except (ValueError, TypeError):
                        pass
            if _my_cards:
                _my_cards = sorted(set(_my_cards))
                st.session_state.clicked_numbers = set(_my_cards)
                all_player_cards = _my_cards
...
# ===================================================================
# ✅ AUTO-CALL NUMBERS — STOPS THE MOMENT A WINNER IS DECLARED
# ===================================================================
if st.session_state.game_started and not st.session_state.winner_declared:
    _, _global_winner_done, _, _, _, _, _, _ = load_global_winners()
    if _global_winner_done:
        sync_global_winners()
        st.rerun()
    else:
...
if st.session_state.game_started and st.session_state.winner_declared:
    # ✅ Do NOT auto-rerun — wait for the player to click Resume.
    pass
