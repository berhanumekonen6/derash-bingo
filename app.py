...
def sync_global_winners():
    ...
    if winner_declared:
        ...
        # 3-second celebration from the moment they first see the winner.
        return True
    return False
...
def try_global_call():
    """✅ CHANGED: Immediately returns None if winner already declared —
       so NO new numbers are called once a winner exists (globally)."""
    # ✅ FIRST CHECK: if winner already declared (from file or session) → stop calling
...
# ===================================================================
BINGO_CARDS = [
    # ⚠️ KEEP YOUR EXISTING FULL 204-CARD LIST HERE — unchanged
...
def distribute_prizes(winners):
    ...
    # ✅ Persist the updated balances to the file
    save_all_data()
...
# ===================================================================
# ✅ WINNER CELEBRATION — 3 SECONDS PER PLAYER (GLOBAL SYNC)
# ===================================================================
if st.session_state.winner_declared:
...
if st.session_state.game_started:
    ...
    # ✅ FALLBACK: Rebuild this user's own cards if the list is empty
...
if st.session_state.game_started and not st.session_state.winner_declared:
    # ✅ Guard: If the global file says a winner exists, DON'T call
...
if st.session_state.game_started and st.session_state.winner_declared:
    # Keep refreshing during celebration so countdown & auto-reset run
    if st.session_state.celebration_start_time:
        elapsed = time.time() - st.session_state.celebration_start_time
        if elapsed < CELEBRATION_DURATION:
            time.sleep(0.5)
