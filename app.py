...
# ... (keep all the functions the same until display_countdown_timer)

def display_countdown_timer():
    """Display timer without auto-rerun - uses JavaScript for live updates"""
    ...
    # Use JavaScript for live timer updates instead of Python rerun
    html = f"""
    <style>
        .timer-container {{
            text-align: center;
    ...
    <div class="timer-container">
        ...
        <div class="timer-refresh">Click refresh button below to update timer</div>
    </div>
    """
...
def display_all_cards_grid():
    """Display all 201 cards with proper selection - FIXED for button interaction"""
    ...
    with col1:
        if st.button("⬅️ Previous", key="prev_page", disabled=st.session_state.board_page == 0):
    ...
    with col3:
        if st.button("Next ➡️", key="next_page", disabled=st.session_state.board_page >= total_pages - 1):
            st.session_state.board_page += 1
            st.rerun()
...
# ===================================================================
# MAIN APP - UPDATED WAITING PHASE
# ===================================================================

# In the main() function, replace the waiting phase section with:
    ...
    if status == "waiting":
        ...
        # Check if countdown ended and game should start - using manual check
        remaining = get_remaining_time()
        if remaining <= 0:
