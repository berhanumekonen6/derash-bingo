...
<style>
    ...
    .small-number-btn[kind="secondary"] {
        background: rgba(255, 215, 0, 0.15) !important;
        color: #FFD700 !important;
        border: 1px solid rgba(255, 215, 0, 0.2) !important;
    }
    
    .small-number-btn[kind="primary"] {
        background: rgba(255, 255, 255, 0.08) !important;
        color: #FFFFFF !important;
        border: 1px solid rgba(255, 255, 255, 0.1) !important;
    }
    
    .small-number-btn:hover {
...
def get_number_sound_js(number):
    """Generate JavaScript to play a beep sound for a number"""
    # Map number to frequency (different pitch for different ranges)
    if 1 <= number <= 15:
        freq = 440  # B range
    elif 16 <= number <= 30:
        freq = 523  # I range
...
def get_winner_sound_js():
    """Generate JavaScript to play a winner celebration sound"""
    return """
    <script>
        (function() {
            try {
...
BINGO_CARDS = [
    ...
    # ... (all 201 cards would be here - truncated for space)
...
def display_selected_card(card_id, called_numbers=None, is_winner=False, winning_pattern=None):
    ...
    <style>
        .card-container-{card_id} {{
            ...
            box-shadow: 0 8px 32px rgba(0,0,0,0.2);
            max-width: 400px;
            border: 2px solid {'#FFD700' if is_winner else 'rgba(255,255,255,0.08)'};
            transition: all 0.3s ease;
        }}
        .card-container-{card_id}:hover {{
            transform: translateY(-2px);
        ...
        .card-table-{card_id} .letter-label {{
        ...
        .card-circle-{card_id}.called {{
            background: rgba(255, 152, 0, 0.25);
            color: #FFD700;
            border-color: #FF9800;
            transform: scale(1.1);
        }}
        .card-circle-{card_id}.free {{
            background: rgba(255, 215, 0, 0.15);
            color: #FFD700;
            font-size: 1.1rem;
            border-color: #FFD700;
        }}
        .card-circle-{card_id}.winner {{
    ...
    html = f'<div class="card-container-{card_id}">'
    html += f'<div class="card-title-{card_id}">🎯 Card #{card_id}</div>'
    
    html += f'<table class="card-table-{card_id}">'
    html += '<tr>'
    for letter in ['B', 'I', 'N', 'G', 'O']:
        html += f'<td class="letter-label">{letter}</td>'
    html += '</tr>'
    ...
    for row_idx in range(5):
        ...
        for col_idx in range(5):
            ...
            if value == 'F':
                html += f'<td><div class="card-circle-{card_id} free">★</div></td>'
            else:
                num = int(value)
                is_called = num in called_numbers
                circle_class = "card-circle-" + str(card_id)
                if is_winner:
                    circle_class += " winner"
                elif is_called:
                    circle_class += " called"
    ...
    if is_winner and winning_pattern:
        html += f'<div class="card-footer-{card_id}" style="color:#FFD700;font-size:0.8rem;">🏆 WINNER! ({winning_pattern}) 🏆</div>'
    else:
        html += f'<div class="card-footer-{card_id}">✅ {total_called}/24 called</div>'
    html += '</div>'
...
def display_master_board():
    ...
    html += '<div class="board-title">🎯 BINGO Board</div>'
...
if not st.session_state.selected_card:
    ...
    st.markdown(f"""
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;flex-wrap:wrap;background:rgba(255,255,255,0.02);padding:8px 15px;border-radius:12px;border:1px solid rgba(255,215,0,0.05);">
    ...
    # Numbers grid - 6 columns with small buttons
    cols = st.columns(6)
        ...
        with cols[col_idx]:
            ...
            <style>
                .btn-{i} {{
                    ...
                    margin: 1px 0 !important;
                    cursor: {'not-allowed' if is_disabled else 'pointer'} !important;
                    opacity: {'0.5' if is_disabled else '1'} !important;
                }}
                .btn-{i}:hover {{
                    transform: scale(1.05);
                }}
    ...
    if len(st.session_state.clicked_numbers) >= 2:
        ...
        st.info("👆 Click a card number to select it (max 2 cards)")
