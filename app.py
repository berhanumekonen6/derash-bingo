...
<style>
    ...
    /* CSS Grid - FORCES the column count */
    .cards-grid {
...
# ===================================================================
# CARD SELECTION FUNCTION - FIXED: Uses pure HTML/CSS grid with HTML buttons
# ===================================================================
...
def render_card_selection():
    ...
    # ================================================================
    # CREATE PURE HTML/CSS GRID WITH HTML BUTTONS
    # ================================================================
    
    cols_per_row = st.session_state.columns_per_row
    
    # Build the HTML for all cards
    cards_html = '<div class="cards-grid-wrapper"><div class="cards-grid" style="grid-template-columns: repeat(' + str(cols_per_row) + ', 1fr) !important;">'
    ...
    for i in range(1, 202):
        ...
        # Create HTML button with onclick handler
        cards_html += f'''
        <div style="width:100%;min-width:0;">
            <button class="grid-card-btn {btn_class}" 
                    onclick="handleCardClick({i})"
                    {"disabled" if is_taken else ""}
                    id="card_{i}">
                {label}
            </button>
        </div>
    ...
    function handleCardClick(cardId) {
        // Send the click to Streamlit using st.query_params or st.rerun with args
        window.location.href = window.location.href.split('?')[0] + '?card=' + cardId;
        // Force a reload
    ...
    # Handle card click via query params
    query_params = st.query_params
    if 'card' in query_params:
        card_id = int(query_params['card'])
